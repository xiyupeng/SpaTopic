...
FREX combines frequency and exclusivity using a weighted harmonic mean. Frequency is the word probabilities ranked and scaled to values between 0 and 1. Then exclusivity is computed by dividing each feature's probability by its total probability across topics, followed by ranking and scaling within each topic. The frequency-exclusivity framework was proposed by Airoldi and Bischof (2016), and the formula used here follows the implementation in the `stm` package (Roberts et al., 2019).
...
```{r bubble_plot, fig.width=8,fig.height=6}
...
```{r circle_packing, fig.width=6,fig.height=6}
...
```{r alltopic_heatmap, fig.width=8,fig.height=6}
