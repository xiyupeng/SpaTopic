...
We first run the model under different number of topics, each collecting 200 posterior samples after 1000 burnin iterations. For higher number of topics, it may take longer time to converge, thus we set `burnin = 2000` instead of the default.

```{r,eval=FALSE}
## PRE-RUN the result
for(topic in 2:9){
  gibbs.res<-SpaTopic_inference(dataset, ntopics = topic, sigma = 50, region_radius = 400, trace = TRUE, thin = 20, burnin = 2000, niter = 200)
  filename<-paste0("~/Documents/Research/github/SpaTopic_data/nanostring_niter200_K",topic,".rds")
