...
vignette: >
  %\VignetteIndexEntry{Xenium Example}
...
We load cell type annotations for this dataset from a previously curated reference and loaded here for visualization and interpretation. You can download the file from [here](https://drive.google.com/drive/folders/1wjc0-0cCAKZtEQqYFBi1OCk969AXNPNb).
...
Load the [file](https://drive.google.com/drive/folders/1wjc0-0cCAKZtEQqYFBi1OCk969AXNPNb) below to skip the data preparation steps above.
...
You can also download the updated object [here](https://drive.google.com/drive/folders/1wjc0-0cCAKZtEQqYFBi1OCk969AXNPNb) and load it to skip these preprocessing steps.
...
In this section, we apply **SpaTopic** to the Xenium breast cancer spatial transcriptomics dataset to infer spatial topics that capture recurrent cellular neighborhood patterns. The data object here can be download from [here](https://cf.10xgenomics.com/samples/xenium/1.0.1/Xenium_FFPE_Human_Breast_Cancer_Rep1/Xenium_FFPE_Human_Breast_Cancer_Rep1_outs.zip). We load the preprocessed Seurat object that contains cell segmentation, cell-type annotations, and spatial coordinates, you can download the file from [here](https://drive.google.com/drive/folders/1wjc0-0cCAKZtEQqYFBi1OCk969AXNPNb).This object will be our input for SpaTopic analysis.
...
We then apply SpaTopic to infer spatial topics across the tissue using `SpaTopic_inference()`. Each topic represents a recurring spatial pattern of cell-type composition. You can download the results from [here](https://drive.google.com/drive/folders/1wjc0-0cCAKZtEQqYFBi1OCk969AXNPNb).
...
In addition to spatial transcriptomics data using Xenium, we also analyze a [10x Chromium scRNA-seq dataset](https://drive.google.com/drive/folders/1oN5lkoUG_MgLm90qG1Ei9wgYTsX3kol-) to identify cell-type composition. We first create a Seurat object using `CreateSeuratObject` function.

```{r, eval=FALSE}
scRNAseq_data<-Read10X("/Users/nikkixiaomengqi/Documents/research/scRNA-seq2/sample_filtered_feature_bc_matrix")
scRNAseq <- CreateSeuratObject(counts = scRNAseq_data, project = "scRNAseq", min.cells = 3, min.features = 200)
```

We can load the [cell-type labels file](https://drive.google.com/drive/folders/1wjc0-0cCAKZtEQqYFBi1OCk969AXNPNb) and attach it to the Seurat object as metadata, using the barcode to match the annotation. The result is showing how many cells belong to each annotated cell type.

```{r, eval=FALSE}
label<-read.csv(file = "/Users/nikkixiaomengqi/Documents/research/scRNAseq_labels.csv")
rownames(label)<-label$Barcode
scRNAseq<-AddMetaData(scRNAseq,label)
scRNAseq$Annotation<-factor(scRNAseq$Annotation)
table(scRNAseq$Annotation)
```
