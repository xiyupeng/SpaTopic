...
**More detailed documentation is available at the [SpaTopic Home Page](https://github.com/xiyupeng/SpaTopic). 
...
`SpaTopic` can be applied on either single images or **across multiple images**.

## Set-up

We use a non-small cell lung cancer image to illustrate . 
The data object here can be download from [here](https://github.com/xiyupeng/SpaTopic),
...
We selected Lung5-1 sample and annotated cells using Azimuth [22] based on the human lung reference v1.0. 
...
## We use Seurat v5 package for visualze the results
library(Seurat, quietly = TRUE);packageVersion("Seurat")
## Load the Seurat object for the image
load("~/github/SpaTopic/data/nanostring_example.rdata")
## for large dataset
...
Please check more examples in [SpaTopic Home Page](https://github.com/xiyupeng/SpaTopic). 
